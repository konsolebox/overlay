...
src_prepare() {
	local PATCHES=(
		...
		"${FILESDIR}"/grub-2.12-match-no-dot-img-initramfs-filenames.patch
	)
...
src_install() {
	...
	insinto /etc/default
	newins "${FILESDIR}"/grub.default-4-konsolebox-1 grub
...
pkg_postinst() {
	...
	if [[ -n ${REPLACING_VERSIONS} ]]; then
		...
		optfeature "detecting UUID through findmnt" sys-apps/util-linux
	fi
