...
SLOT="0"
IUSE="apidoc bittorrent brotli bzip2 debug doc finger ftp gopher gpm gnutls
	guile idn javascript lua lzma +mouse nls nntp perl python samba ssl test tre
...
src_install() {
	...
	_dodoc_if_use() { use "$1" || return ; docinto "$2" ; dodoc "${@:3}" ; }
	_dodoc_if_use guile contrib/guile contrib/guile/*.scm
	_dodoc_if_use lua contrib/lua contrib/lua/{*.lua,elinks-remote}
	_dodoc_if_use python contrib/python contrib/python/*.py
	unset -f _dodoc_if_use
