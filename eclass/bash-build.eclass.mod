...
SLOTTED_SLOT=${PV/_p/.} SLOTTED_SLOT=${SLOTTED_SLOT//_/-}
...
[[ ${PN} != bash || ${SLOT} != "${PV}" ]] && RDEPEND+="!app-shells/bash:${PV}"$'\n'

# If app-shells/bash:0 conflicts with app-shells/bash:${SLOTTED_SLOT}: (Just for reference)
# [[ ${PN} == bash && ${SLOT} == 0 && ${SLOTTED_SLOT} != "${PV}" ]] && RDEPEND+="!app-shells/bash:${SLOTTED_SLOT}"$'\n'

# If app-shells/bash conflicts with app-shells/bash-slotted:
# [[ ${PN} != bash-slotted ]] && RDEPEND+="!app-shells/bash-slotted:${SLOTTED_SLOT}"$'\n'
