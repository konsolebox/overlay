...
# @ECLASS: distutils-r1-eaee61e2.eclass
# @MAINTAINER:
...
#
# In distutils-r1-eaee61e2_src_prepare(), the 'all' function is run before
...
# the default one will be used (distutils-r1-eaee61e2_...). Defaults are provided
...
#
# Please note that distutils-r1-eaee61e2 sets RDEPEND and BDEPEND (or DEPEND
# in earlier EAPIs) unconditionally for you.
#
# Also, please note that distutils-r1-eaee61e2 will always inherit python-r1
# as well. Thus, all the variables defined and documented there are
# relevant to the packages using distutils-r1-eaee61e2.
#
...
# distutils-r1-eaee61e2 default phase functions or call the build system
# manually.
#
...
# by python-r1.eclass.  Otherwise, it is set by distutils-r1-eaee61e2.eclass
# for consistency.
#
# Example value:
# @CODE
...
_distutils_set_globals() {
	...
	if [[ ${DISTUTILS_USE_PEP517} ]]; then
		...
		eqawarn "QA Notice: distutils-r1-eaee61e2.eclass legacy mode is deprecated and will be removed."
...
distutils_enable_tests() {
	...
	python_test() { distutils-r1-eaee61e2_python_test; }
...
# in distutils-r1-eaee61e2_python_compile and in distutils-r1-eaee61e2_python_install.
#
...
esetup.py() {
	...
	if [[ ${BUILD_DIR} && ! ${DISTUTILS_USE_PEP517} ]]; then
		_distutils-r1-eaee61e2_create_setup_cfg
	fi
...
# @FUNCTION: _distutils-r1-eaee61e2_check_all_phase_mismatch
# @INTERNAL
# @DESCRIPTION:
# Verify whether *_all phase impls is not called from from non-*_all
# subphase.
_distutils-r1-eaee61e2_check_all_phase_mismatch() {
	if has "python_${EBUILD_PHASE}" "${FUNCNAME[@]}"; then
		eqawarn "QA Notice: distutils-r1-eaee61e2_python_${EBUILD_PHASE}_all called"
		eqawarn "from python_${EBUILD_PHASE}.  Did you mean to use"
		eqawarn "python_${EBUILD_PHASE}_all()?"
		[[ ${EAPI} != 7 ]] &&
			die "distutils-r1-eaee61e2_python_${EBUILD_PHASE}_all called from python_${EBUILD_PHASE}."
	fi
}

# @FUNCTION: _distutils-r1-eaee61e2_print_package_versions
# @INTERNAL
# @DESCRIPTION:
# Print the version of the relevant build system packages to aid
# debugging.
_distutils-r1-eaee61e2_print_package_versions() {
	local packages=()
...
# @FUNCTION: distutils-r1-eaee61e2_python_prepare_all
# @DESCRIPTION:
...
# @FUNCTION: _distutils-r1-eaee61e2_create_setup_cfg
# @INTERNAL
# @DESCRIPTION:
...
# @FUNCTION: _distutils-r1-eaee61e2_copy_egg_info
# @INTERNAL
# @DESCRIPTION:
...
# egg-info.
_distutils-r1-eaee61e2_copy_egg_info() {
	if [[ ${DISTUTILS_USE_PEP517} ]]; then
		die "${FUNCNAME} is not implemented in PEP517 mode"
	fi
...
# @FUNCTION: _distutils-r1-eaee61e2_key_to_backend
# @USAGE: <key>
# @INTERNAL
# @DESCRIPTION:
# Print the backend corresponding to the DISTUTILS_USE_PEP517 value.
_distutils-r1-eaee61e2_key_to_backend() {
	debug-print-function ${FUNCNAME} "$@"
...
_distutils-r1-eaee61e2_get_backend() {
	...
	echo "$(_distutils-r1-eaee61e2_key_to_backend "${DISTUTILS_USE_PEP517}")"
...
distutils_pep517_install() {
	...
	local build_backend=$(_distutils-r1-eaee61e2_get_backend)
...
# of distutils-r1-eaee61e2_python_compile invocations, mapped to the source
...
# @FUNCTION: distutils-r1-eaee61e2_python_compile
...
distutils-r1-eaee61e2_python_compile() {
	debug-print-function ${FUNCNAME} "$@"
	...
	# distutils-r1-eaee61e2_python_compile can be called multiple
...
# @FUNCTION: _distutils-r1-eaee61e2_wrap_scripts
# @USAGE: <bindir>
# @INTERNAL
# @DESCRIPTION:
# Moves and wraps all installed scripts/executables as necessary.
_distutils-r1-eaee61e2_wrap_scripts() {
	debug-print-function ${FUNCNAME} "$@"
...
# @FUNCTION: distutils-r1-eaee61e2_python_test
# @USAGE: [additional-args...]
# @DESCRIPTION:
...
distutils-r1-eaee61e2_python_test() {
	debug-print-function ${FUNCNAME} "$@"
...
# @FUNCTION: distutils-r1-eaee61e2_python_install
# @USAGE: [additional-args...]
# @DESCRIPTION:
...
distutils-r1-eaee61e2_python_install() {
	...
	if [[ ${DISTUTILS_USE_PEP517} ]]; then
		...
		# we are assuming that _distutils-r1-eaee61e2_post_python_compile
	...
	if [[ ! ${DISTUTILS_SINGLE_IMPL} ]]; then
		_distutils-r1-eaee61e2_wrap_scripts "${scriptdir}"
	fi
}

# @FUNCTION: distutils-r1-eaee61e2_python_install_all
# @DESCRIPTION:
...
# @FUNCTION: distutils-r1-eaee61e2_run_phase
# @USAGE: [<argv>...]
# @INTERNAL
# @DESCRIPTION:
...
distutils-r1-eaee61e2_run_phase() {
	debug-print-function ${FUNCNAME} "$@"
	...
	if [[ ! ${_DISTUTILS_IN_COMMON_IMPL} ]] &&
		declare -f "_distutils-r1-eaee61e2_post_python_${EBUILD_PHASE}" >/dev/null
	then
		"_distutils-r1-eaee61e2_post_python_${EBUILD_PHASE}"
	fi
	return "${ret}"
}

# @FUNCTION: _distutils-r1-eaee61e2_run_common_phase
# @USAGE: [<argv>...]
# @INTERNAL
# @DESCRIPTION:
...
_distutils-r1-eaee61e2_run_common_phase() {
	local DISTUTILS_ORIG_BUILD_DIR=${BUILD_DIR}
	local _DISTUTILS_IN_COMMON_IMPL=1

	if [[ ${DISTUTILS_SINGLE_IMPL} ]]; then
		# reuse the dedicated code branch
		_distutils-r1-eaee61e2_run_foreach_impl "${@}"
	else
		...
		# store for restoring after distutils-r1-eaee61e2_run_phase.
...
# @FUNCTION: _distutils-r1-eaee61e2_run_foreach_impl
# @INTERNAL
# @DESCRIPTION:
...
_distutils-r1-eaee61e2_run_foreach_impl() {
	...
	# store for restoring after distutils-r1-eaee61e2_run_phase.
	local _DISTUTILS_INITIAL_CWD=${PWD}
	set -- distutils-r1-eaee61e2_run_phase "${@}"
...
distutils-r1-eaee61e2_src_prepare() {
	debug-print-function ${FUNCNAME} "$@"
	local ret=0
	...
	if declare -f python_prepare_all >/dev/null; then
		...
		distutils-r1-eaee61e2_python_prepare_all || ret=${?}
	fi

	if [[ ! ${_DISTUTILS_DEFAULT_CALLED} ]]; then
		die "QA: python_prepare_all() didn't call distutils-r1-eaee61e2_python_prepare_all"
	fi

	if declare -f python_prepare >/dev/null; then
		_distutils-r1-eaee61e2_run_foreach_impl python_prepare || ret=${?}
	fi

	return ${ret}
}

distutils-r1-eaee61e2_src_configure() {
	debug-print-function ${FUNCNAME} "$@"
	local ret=0

	if declare -f python_configure >/dev/null; then
		_distutils-r1-eaee61e2_run_foreach_impl python_configure || ret=${?}
	fi

	if declare -f python_configure_all >/dev/null; then
		_distutils-r1-eaee61e2_run_common_phase python_configure_all || ret=${?}
	fi

	return ${ret}
}

# @FUNCTION: _distutils-r1-eaee61e2_compare_installed_files
# @INTERNAL
# @DESCRIPTION:
# Verify the the match between files installed between this and previous
# implementation.
_distutils-r1-eaee61e2_compare_installed_files() {
	debug-print-function ${FUNCNAME} "$@"
...
# @FUNCTION: _distutils-r1-eaee61e2_post_python_compile
# @INTERNAL
# @DESCRIPTION:
# Post-phase function called after python_compile.  In PEP517 mode,
# it adjusts the install tree for venv-style usage.
_distutils-r1-eaee61e2_post_python_compile() {
	debug-print-function ${FUNCNAME} "$@"

	if [[ ! ${_DISTUTILS_WHL_INSTALLED} && ${DISTUTILS_USE_PEP517:-no} != no ]]
	then
		die "No wheel installed in python_compile(), did you call distutils-r1-eaee61e2_python_compile?"
	fi
...
distutils-r1-eaee61e2_src_compile() {
	debug-print-function ${FUNCNAME} "$@"
	local ret=0

	if declare -f python_compile >/dev/null; then
		_distutils-r1-eaee61e2_run_foreach_impl python_compile || ret=${?}
	else
		_distutils-r1-eaee61e2_run_foreach_impl distutils-r1-eaee61e2_python_compile || ret=${?}
	fi

	if declare -f python_compile_all >/dev/null; then
		_distutils-r1-eaee61e2_run_common_phase python_compile_all || ret=${?}
	fi

	return ${ret}
}

# @FUNCTION: _distutils-r1-eaee61e2_clean_egg_info
# @INTERNAL
# @DESCRIPTION:
...
_distutils-r1-eaee61e2_clean_egg_info() {
	if [[ ${DISTUTILS_USE_PEP517} ]]; then
		die "${FUNCNAME} is not implemented in PEP517 mode"
	fi
...
# @FUNCTION: _distutils-r1-eaee61e2_post_python_test
# @INTERNAL
# @DESCRIPTION:
# Post-phase function called after python_test.
_distutils-r1-eaee61e2_post_python_test() {
	debug-print-function ${FUNCNAME} "$@"
...
distutils-r1-eaee61e2_src_test() {
	debug-print-function ${FUNCNAME} "$@"
	local ret=0

	if declare -f python_test >/dev/null; then
		_distutils-r1-eaee61e2_run_foreach_impl python_test || ret=${?}
	fi

	if declare -f python_test_all >/dev/null; then
		_distutils-r1-eaee61e2_run_common_phase python_test_all || ret=${?}
	fi

	return ${ret}
}

# @FUNCTION: _distutils-r1-eaee61e2_strip_namespace_packages
# @USAGE: <sitedir>
# @INTERNAL
# @DESCRIPTION:
# Find and remove setuptools-style namespaces in the specified
# directory.
_distutils-r1-eaee61e2_strip_namespace_packages() {
	debug-print-function ${FUNCNAME} "$@"
...
# @FUNCTION: _distutils-r1-eaee61e2_post_python_install
# @INTERNAL
# @DESCRIPTION:
...
_distutils-r1-eaee61e2_post_python_install() {
	debug-print-function ${FUNCNAME} "$@"

	local sitedir=${D}$(python_get_sitedir)
	if [[ -d ${sitedir} ]]; then
		_distutils-r1-eaee61e2_strip_namespace_packages "${sitedir}"
...
distutils-r1-eaee61e2_src_install() {
	debug-print-function ${FUNCNAME} "$@"
	local ret=0

	if declare -f python_install >/dev/null; then
		_distutils-r1-eaee61e2_run_foreach_impl python_install || ret=${?}
	else
		_distutils-r1-eaee61e2_run_foreach_impl distutils-r1-eaee61e2_python_install || ret=${?}
	fi

	if declare -f python_install_all >/dev/null; then
		_distutils-r1-eaee61e2_run_common_phase python_install_all || ret=${?}
	else
		_distutils-r1-eaee61e2_run_common_phase distutils-r1-eaee61e2_python_install_all || ret=${?}
	fi
