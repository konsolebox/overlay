...
# @FUNCTION: _rubyexec-r1_priority_sort_impls
# @USAGE: <impl> ...
# @DESCRIPTION:
# Produces a list of ruby implementations sorted by priority
# @INTERNAL
_rubyexec-r1_priority_sort_impls() {
	local impl i
	_sorted_impls=("$1")
	shift
...
# @DESCRIPTION:
# Parses commoon options passed to rubyexec-r1_install_fakegem_binwrapper[s]
# @INTERNAL
_rubyexec-r1_parse_common_options() {
	OPTSHIFT=0
...
# @FUNCTION: _rubyexec-r1_create_fakegem_binwrapper
...
# @INTERNAL
_rubyexec-r1_create_fakegem_binwrapper() {
	...
	if [[ ${5+.} ]]; then
		...
		lines+=("require 'rubygems'" "load Gem.activate_path('${gem_name}', '${binary_name}')")
	fi

	printf '%s\n' > "${path}" "${lines[@]}" || die "Failed to write binwrapper code to '${path}'."
}

# @FUNCTION: rubyexec-r1_install_fakegem_binwrappers
# @USAGE: <binary_name> <impl> ... [<option> [<optarg>]] ...
# @DESCRIPTION:
# Creates and installs a single binary wrapper
rubyexec-r1_install_fakegem_binwrapper() {
	...
	while [[ $# -gt 0 ]]; do
		if _rubyexec-r1_parse_common_options "${@:1:2}"; then
			shift "${OPTSHIFT}"
		elif [[ $1 == -* ]]; then
	...
	[[ ${impls+.} ]] || die "No implementations specified."

	if [[ ${autopick} == true ]]; then
		_rubyexec-r1_priority_sort_impls "${impls[@]}"
	...
	temp=${T}/rubyexec-r1_bin-${binary_name}

	if [[ ${autoversion} == true ]]; then
		_rubyexec-r1_create_fakegem_binwrapper "${temp}" "${new_header}" "${binary_name}" "${gem_name}"
	else
		_rubyexec-r1_create_fakegem_binwrapper "${temp}" "${new_header}" "${binary_name}" "${gem_name}"
...
rubyexec-r1_install_fakegem_binwrappers() {
	local all_binaries autopick=false autoversion=false binary binary_name bindirs content \
			extglob_was_enabled=false final_new_header gem_bindir gem_name gem_version \
			globstar_was_enabled=false impls=() new_header no_skip_ruby_exec=false rubyexec_arg \
			temp _sorted_impls
	...
	while [[ $# -gt 0 ]]; do
		if _rubyexec-r1_parse_common_options "${@:1:2}"; then
			shift "${OPTSHIFT}"
		elif [[ $1 == --no-skip-ruby-exec ]]; then
			no_skip_ruby_exec=true
		elif [[ $1 == -* ]]; then
			die "Invalid option: $1"
		else
			impls+=("$1")
	...
	# Use the ruby implementation directly if there's only one and --no-skip-ruby-exec isn't
	# specified.

	if [[ ${#impls[@]} -eq 1 && ${no_skip_ruby_exec} == false ]]; then
		new_header="#!/usr/bin/ruby${impls}"
	else
		# Autopick allows other non-selected version which are still in the list to be used.
	...
	for binary in "${bindirs}"/*; do
		...
		if [[ ${#impls[@]} -eq 1 ]] || _rubyexec-r1_all_files_the_same "${all_binaries[@]}" &&
		...
		# Autoversion isn't really sensible to be used here but added anyway for completeness.
			...
			_rubyexec-r1_create_fakegem_binwrapper "${temp}" "${new_header}" "${binary_name}" \
					"${gem_name}"
		else
			_rubyexec-r1_create_fakegem_binwrapper "${temp}" "${new_header}" "${binary_name}" \
