...
src_prepare() {
	...
	if ! use vanilla; then
		...
		xzcat "${FILESDIR}/${PN}-0.68.0-command-notify.patch.xz" > "${WORKDIR}"/${PN}-0.68.0-command-notify.patch || die
		du -sh "${WORKDIR}"/${PN}-0.68.0-command-notify.patch
