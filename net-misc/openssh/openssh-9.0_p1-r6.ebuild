...
SRC_URI="mirror://openbsd/OpenSSH/portable/${PARCH}.tar.gz
	...
	${HPN_VER:+hpn? ( $(printf "https://downloads.sourceforge.net/project/hpnssh/Patches/HPN-SSH%%20${HPN_VER/./v}%%20${HPN_PV/_P/p}/%s\n" "${HPN_PATCHES[@]}") )}
...
IUSE="abi_mips_n32 asterisk-patch audit debug hpn kerberos ldns libedit livecd pam +pie sctp security-key selinux +ssl static test X X509 xmss"
...
BDEPEND="
	|| ( sys-devel/autoconf dev-build/autoconf )
	virtual/pkgconfig
	|| (
		>=sys-devel/gcc-config-2.6
...
src_prepare() {
	...
	# apply asterisks patch
	use asterisk-patch && eapply "${FILESDIR}/${PN}-8.6_p1-asterisks.patch"
