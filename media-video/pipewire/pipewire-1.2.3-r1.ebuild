...
multilib_src_install_all() {
	...
	if use system-service; then
		newtmpfiles - pipewire.conf <<-EOF || die
			d /run/pulse 0750 pipewire pipewire -
			f /run/pulse/pid 0660 pipewire pipewire -
			f /run/pulse/cookie 0660 pipewire pipewire -
		EOF

		systemd_dounit "${FILESDIR}"/systemd/system/pipewire-pulse.{service,socket}

		insinto /etc/pulse/client.conf.d
		doins "${FILESDIR}"/00-pipewire-pulse-system-client.conf

		# Better remove user unit files.
		local user_unit_dir=$(systemd_get_userunitdir)
		rm -f "${ED}/${user_unit_dir}"/pipewire{,-pulse}.{service,socket} || die
	fi
...
pkg_postinst() {
	...
	if [[ ${HAD_SOUND_SERVER} -eq 0 || -z ${REPLACING_VERSIONS} ]]; then
		...
		if use sound-server && use systemd; then
			...
			elog
			elog "  systemctl --user stop pulseaudio.service pulseaudio.socket"
			elog "  systemctl --user disable pulseaudio.service pulseaudio.socket"
			elog
			elog "A reboot may help ensure interferences from still running PulseAudio"
			elog "daemon are avoided."
			elog

			if use system-service; then
				elog "To use PipeWire, the system units must be manually enabled."
				elog
				elog "  systemctl enable pipewire.service pipewire.socket"
				elog "  systemctl enable pipewire-pulse.service pipewire-pulse.socket"
				elog
				elog "Both new users and those upgrading need to enable WirePlumber"
				elog
				elog "  systemctl disable pipewire-media-session.service"
				elog "  systemctl --force enable wireplumber.service"
			else
				elog "To use PipeWire, the user units must be manually enabled"
				elog "by running this command as each user you use for desktop activities:"
				elog
				elog "  systemctl --user enable pipewire.service pipewire.socket"
				elog "  systemctl --user enable pipewire-pulse.service pipewire-pulse.socket"
				elog
				elog "Both new users and those upgrading need to enable WirePlumber"
	...
	if use system-service && [[ ${HAD_SYSTEM_SERVICE} -eq 0 || -z ${REPLACING_VERSIONS} ]]; then
		ewarn
		ewarn "You have enabled the system-service USE flag, which installs the"
		ewarn "system-wide systemd units that enable PipeWire to run as a system"
		ewarn "service. Default configuration files may not work right away or"
		ewarn "conflict with existing configuration. Default configuration also"
		ewarn "requires pipewire and pipewire-pulse service users to be added to the"
		ewarn "pipewire group."
		ewarn
	fi
